    ...
    for key, csv_files in csv_files_dict.items():
        ...
        for csv_file in csv_files:
            ...
            if match:
                ...
                plt.figure()
                plt.plot(df['Day'], df['LaeqD'], label='LaeqD', marker='o')
                plt.plot(df['Day'], df['LaeqN'], label='LaeqN', marker='o')
                plt.xlabel('Day')
...
def create_excel_with_yearly_charts(csv_files_dict, avg_df_dict):
    for key, csv_files in csv_files_dict.items():
        ...
        plt.figure()
        plt.plot(avg_df['Month'], avg_df['Avg_LaeqD'], label='Avg_LaeqD', marker='o')
        plt.plot(avg_df['Month'], avg_df['Avg_LaeqN'], label='Avg_LaeqN', marker='o')
        plt.xlabel('Month')
...
def create_excel_with_long_term_charts(csv_files_dict, avg_df, output_excel):
    ...
    plt.figure()
    plt.plot(avg_df['Date'], avg_df['Avg_LaeqD'], label='Avg_LaeqD', marker='o')
    plt.plot(avg_df['Date'], avg_df['Avg_LaeqN'], label='Avg_LaeqN', marker='o')
    plt.xlabel('Month')
    plt.title(sheet_name)
    plt.ylabel('Level (dB)')
    plt.legend()
